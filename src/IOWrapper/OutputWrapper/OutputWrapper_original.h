...
namespace IOWrap
{
class OutputWrapper_original: public Output3DWrapper
{
public:
        inline OutputWrapper_original()
        {
            printf("OUT: Created OutputWrapper_original\n");
        }

        virtual ~OutputWrapper_original()
        {
            printf("OUT: Destroyed OutputWrapper_original\n");
        }
         ...
         */
         /*・[CalibHessian]-> fxl（）、fyl（）、cxl（）、cyl（
           *・[PointHessian]
           *-> u、v：ポイントのピクセル座標。
           *-> idepth_scaled：ポイントの逆深度。
           * x = (u-cxl())*z/fxl()
           * y = (v-cyl())*z/fyl()
           * z = 1/idepth_scaled()
           */
         virtual void publishKeyframes(std::vector<FrameHessian*> &frames, bool final, CalibHessian* HCalib) override
         {
               ...
               std::ofstream myfile("result_xyz.txt", std::ios::app);
               float fu = 0;
               float fv = 0;
               float cu = 0;
               float cv = 0;/*どこで初期化するべきか*/
               float x = 0;
               float y = 0;
               float z = 0;
             /* if(final)*/
              /* {*/
               ...
               cu = HCalib-> cxl();
               cv = HCalib-> cyl();
               auto & m =  f->shell->camToWorld.matrix3x4();
               /*auto const& points = ;*/
